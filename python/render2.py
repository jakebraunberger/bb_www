...
def update_current_charts(txt):
	...
	conn = create_connection('./home/ee/bb_www/python/databases/display.db')
...
def update_errors(txt):
	conn = create_connection('./home/ee/bb_www/python/databases/errorlogs.db')
...
def read_template_and_write_html(data_to_input):
	...
	#.  html file
	file = glob.glob('./home/ee/bb_www/python/*html')
	if (len(file) != 0):
		file = file[0]
	fi = open('./home/ee/bb_www/python/template/index.html', 'r')
	...
	fo = open('./home/ee/bb_www/python/%s.html' % fo_name, 'w')
	fo.write(out_text)
	fo.close()
